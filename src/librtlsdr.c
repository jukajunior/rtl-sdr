 ...
 * FIR coefficients.
 *
 * The filter is running at XTal frequency. It is symetric filter with 32
 * coefficients. Only first 16 coefficients are specified, the other 16
 * use the same values but in reversed order. The most left coefficient is
 * the outer one, the last is inner and is 0.5 taps away from axis of symetry.
 * First 8 coefficients are 8 bit signed integers, the following 8 coefficients
...
static const int fir_default[FIR_LEN] = {
 ...
 /* ^ outermost, 8 bit signed             | 12 bit signed,           innermost ^ */
};
